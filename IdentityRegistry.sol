...
contract IdentityRegistry is AccessControlDefaultAdminRules, IIdentityRegistry {
     ...
     */
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "IdentityRegistry: caller is not an admin");
        _;
    }
    ...
    constructor(
        ...
        require(_maxWalletsPerIdentity > 0, "IdentityRegistry: max wallets must be greater than zero");
        require(_initialAdmin != address(0), "IdentityRegistry: initial admin cannot be zero address");
    ...
    function registerIdentity(
        ...
        require(wallets.length <= maxWalletsPerIdentity, "IdentityRegistry: exceeds maximum wallets per identity");
        require(wallets.length > 0, "IdentityRegistry: must provide at least one wallet");
        require(expiryDate > block.timestamp, "IdentityRegistry: expiry date must be in the future");
        
        Identity storage newIdentity = identities[hashTx];
        require(newIdentity.expiryDate == 0, "IdentityRegistry: identity already exists");

        // Check if all wallet addresses are available for registration
        for (uint256 i = 0; i < wallets.length; i++) {
            require(wallets[i] != address(0), "IdentityRegistry: wallet cannot be zero address");
            require(addressToHashTx[wallets[i]] == bytes32(0), "IdentityRegistry: wallet already registered");
    ...
    function registerIdentities(
        ...
        require(length == countries.length, "IdentityRegistry: countries length mismatch");
        require(length == datas.length, "IdentityRegistry: datas length mismatch");
        require(length == expiryDates.length, "IdentityRegistry: expiry dates length mismatch");
        require(length == walletsLists.length, "IdentityRegistry: wallets lists length mismatch");

        for (uint256 j = 0; j < length; j++) {
            bytes32 hashTx = hashTxs[j];
            require(hashTx != bytes32(0), "IdentityRegistry: hash cannot be zero");
            require(expiryDates[j] > block.timestamp, "IdentityRegistry: expiry date must be in the future");
            require(walletsLists[j].length <= maxWalletsPerIdentity, "IdentityRegistry: exceeds maximum wallets per identity");
            require(walletsLists[j].length > 0, "IdentityRegistry: must provide at least one wallet");
            ...
            for (uint256 i = 0; i < walletsLists[j].length; i++) {
                ...
                require(addressToHashTx[wallet] == bytes32(0), "IdentityRegistry: wallet already registered");
            ...
            emit IdentityRegistered(hashTx, expiryDates[j], walletsLists[j], countries[j], datas[j]);
        }
    }
    ...
    function deleteIdentity(bytes32 hashTx) external onlyAdmin {
        ...
        require(identity.expiryDate > 0, "IdentityRegistry: identity not found");
    ...
    function addWallets(bytes32 hashTx, address[] memory newWallets) external onlyAdmin {
        require(newWallets.length > 0, "IdentityRegistry: must provide at least one wallet");
        
        Identity storage identity = identities[hashTx];
        require(identity.expiryDate > 0, "IdentityRegistry: identity not found");
        require(identity.expiryDate > block.timestamp, "IdentityRegistry: identity has expired");
        require(
            identity.wallets.length + newWallets.length <= maxWalletsPerIdentity,
        ...
        for (uint256 j = 0; j < newWallets.length; j++) {
            ...
            require(newWallet != address(0), "IdentityRegistry: wallet cannot be zero address");
            require(addressToHashTx[newWallet] == bytes32(0), "IdentityRegistry: wallet already registered");
    ...
    function removeWallets(bytes32 hashTx, address[] memory walletsToRemove) external onlyAdmin {
        require(walletsToRemove.length > 0, "IdentityRegistry: must provide at least one wallet to remove");
        
        Identity storage identity = identities[hashTx];
        require(identity.expiryDate > 0, "IdentityRegistry: identity not found");
        require(identity.wallets.length > walletsToRemove.length, "IdentityRegistry: cannot remove all wallets");

        // Remove wallet addresses
        for (uint256 j = 0; j < walletsToRemove.length; j++) {
            address walletToRemove = walletsToRemove[j];
            require(addressToHashTx[walletToRemove] == hashTx, "IdentityRegistry: wallet not associated with identity");
            
            // Find the wallet address in the array
            uint256 index = 0;
            while (index < identity.wallets.length && identity.wallets[index] != walletToRemove) {
                index++;
            }
            require(index < identity.wallets.length, "IdentityRegistry: wallet not found in identity");
    ...
    function updateExpiryDate(bytes32 hashTx, uint256 newExpiryDate) external onlyAdmin {
        Identity storage identity = identities[hashTx];
        require(identity.expiryDate > 0, "IdentityRegistry: identity not found");
        require(newExpiryDate > block.timestamp, "IdentityRegistry: new expiry date must be in the future");
